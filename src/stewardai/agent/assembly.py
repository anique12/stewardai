...
_TOOLS_AVAILABLE_NOTE = (
    ...
    "- Speech-to-text OFTEN MISHEARS spelled-out or precise values — email addresses, "
    "URLs, domains, phone numbers, exact names. Before you act on such a value, READ IT "
    "BACK the way you understood it (e.g. \"that's john at propellus dot c-o — correct?\") "
    "and wait for confirmation; if they correct it, use the correction. Never send or "
    "file an email to an address you are not confident you heard correctly — the live "
...
def build_meeting_system(
    ...
    *,
    ...
    base = (
        ...
        "to speak.\n"
        f'- You WAKE only when someone says your name ("{name}"). A question or request '
        "that does NOT include your name is NOT for you — it is the people in the "
        "meeting talking to each other — even if it sounds like a direct question. In "
        "that case call stay_silent.\n"
        "- Once someone wakes you by name, you are in a conversation with THAT person: "
        "treat their immediate follow-ups as the SAME conversation and keep answering "
        "them — even if they don't repeat your name, even if it repeats an earlier "
        'question, or it is just a check like "can you hear me?". A person in a '
        "conversation with you expects a reply every turn.\n"
        "- On EACH turn, judge whether the conversation is STILL with you, and call "
        "stay_silent when it is NOT. It is no longer with you when ANY of these "
        "happen: the topic clearly changes or the discussion moves to other people "
        "talking to EACH OTHER; someone addresses another participant BY NAME (that "
        "question is for them, not you); a general question is asked that is not aimed "
        "at you; or you thought a question was for you but another participant answers "
