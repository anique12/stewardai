...
// like AttendeeAvatars and PersonAvatar). `gravatarUrl` below uses a
// dependency-free MD5 implementation (rather than Node's `crypto`) precisely
// so it can run in the browser bundle. ./attendees.ts re-exports it for
// server-side callers (e.g. calendar sync) that already import from there.
...
// --- Dependency-free MD5 (public-domain algorithm, Joseph Myers) ----------
// Only used to build Gravatar URLs client-side; not for anything
// security-sensitive.
...
function md5(input: string): string {
  // Encode to UTF-8 bytes-as-chars first (the algorithm above assumes 8-bit chars).
