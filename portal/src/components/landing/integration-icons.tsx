// Official brand marks for the integrations the agent acts in. Path data and
// colors are inlined VERBATIM from official brand SVG assets (not redrawn):
//   - Google Calendar: Wikimedia Commons "Google Calendar icon (2020)" SVG
//     (viewBox 0 0 200 200, multi-color)
//   - Gmail: Wikimedia Commons "Gmail icon (2020)" SVG
//     (viewBox 52 42 88 66, multi-color)
//   - Slack: Wikimedia Commons "Slack icon 2019" SVG (4-color clover)
//   - Notion: simple-icons official single-color mark (rendered white so the
//     black glyph reads on the dark theme)
// Multi-color marks sit on small white rounded tiles so they read cleanly.
// Third-party trademarks shown only to indicate supported integrations
// (nominative fair use).

type IconProps = { className?: string };

// Google Calendar — official multi-color mark (Wikimedia "Google Calendar icon
// (2020)"). The source nests its paths under a translate(3.75 3.75) group; kept
// verbatim so the coordinates stay exact against the 0 0 200 200 viewBox.
...
// Gmail — official multi-color mark (Wikimedia "Gmail icon (2020)"). viewBox is
// the source's 52 42 88 66 so the envelope is centered.
...
// Notion — official single-color mark (simple-icons). Source is a black glyph
// on a 0 0 24 24 viewBox. The other three marks sit on white tiles, so a white
// Notion glyph alone would vanish; we keep the white glyph but seat it on a
// dark rounded square baked into this SVG so it reads on the shared white tile
// while staying the canonical Notion treatment.
export function NotionIcon({ className }: IconProps) {
  return (
    <svg viewBox="-2 -2 28 28" className={className} aria-hidden>
      <rect x="-2" y="-2" width="28" height="28" rx="5" fill="#0f0f0f" />
      <path
        fill="#fff"
...
// Slack — official 4-color clover mark (Wikimedia "Slack icon 2019"). Source is
// 127x127; viewBox set to 0 0 127 127 to match.
