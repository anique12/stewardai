...
// under it. `pending` marks the act-on-request example, whose reply is
// framed as the product vision (rolling out) rather than something every
// app supports today.
type Exchange = {
  you: string;
  aloud: string;
...
const EXCHANGES: Exchange[] = [
  {
    you: "MeetBase, what did we decide about pricing?",
    aloud:
  ...
  {
    you: "MeetBase, summarize where we are.",
    aloud:
  ...
  {
    you: "MeetBase, log a follow-up with Priya for Friday.",
    aloud: "On it — I'll add a follow-up action item for Friday and note it in this meeting's Space.",
        ...
        <div className="mt-12 grid items-start gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:gap-14">
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="px-1 text-xs leading-relaxed text-muted-foreground/70">
              While other tools quietly transcribe in the background, MeetBase is the
              one teammate in the call you can actually talk to.
            </p>
          </div>

          {/* Right: the spoken exchanges — a "you ↔ MeetBase" conversation. */}
          <div className="card-ring overflow-hidden rounded-2xl shadow-2xl shadow-black/40">
            ...
            <ul className="flex flex-col gap-6 p-5 sm:p-6">
              {EXCHANGES.map((ex) => (
                <li key={ex.you} className="flex flex-col gap-2.5">
                  ...
                  {/* MeetBase — left-aligned, voice orb + waveform, spoken aloud. */}
            ...
            <div className="flex flex-col items-start gap-3 border-t border-border bg-background/40 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
              <p className="text-xs leading-relaxed text-muted-foreground">
                Speaking in meetings is optional — turn it on per meeting, or leave MeetBase silent and just taking notes.
              </p>
            </div>
          </div>
        </div>
...
// A small, hollow voice orb echoing the "speaks in meetings" visualizer: two
// concentric rings with a soft glow and a gentle live pulse, marking lines
// MeetBase speaks out loud. Purely decorative.
